...
class Cities
{
    ...
    public function nextStep($turn)
    {
        ...
        if (mb_strlen($search) <= 2) {
            return 'Текущий город: ' . mb_strtoupper($this->used[$this->count1], 'UTF-8') . ' Первая буква города должна быть ' . $this->current;
        }
        ...
        if (mb_strpos($this->current, $firstletter) !== false) //check if the letter is correct
        {
            foreach ($this->lines as $line) {
                ...
                if (mb_strtolower($fndcity) === mb_strtolower($search)) //if the city exists
                {
                    ...
                    if (is_null($this->checkLetter ($newletter))) {
                        $this->removeGame();
                        return "Не осталось городов. Игра окончена!";
                    } else {
